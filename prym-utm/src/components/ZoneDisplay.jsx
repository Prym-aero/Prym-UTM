...
const ZoneDisplay = ({ zone }) => {
  ...
  // ✅ Helper function to render zone details
  const renderZoneDetails = () => {
    if (!selectedZone) return null;

    return (
      <div
        className={`absolute z-[500] bg-white shadow-lg transition-all duration-200 pb-5 overflow-y-auto px-5 w-[388px] h-[80%] rounded-lg top-16 right-10`}
      >
        {/* Close Button */}
        <IoClose
  ...
  // ✅ Helper function to render a single zone
  ...
  return (
    <>
      {/* Render all zones */}
      {zone.map((zone, index) => renderZone(zone, index))}
